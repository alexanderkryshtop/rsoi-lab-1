...
func (p *PersonRepository) Create(model *model.Person) (uint64, error) {
	...
	err := p.dbPool.QueryRow(context.Background(),
		"INSER INTO persons (name, age, address, work) VALUES($1, $2, $3, $4) RETURNING id",
...
func (p *PersonRepository) GetAll() ([]model.Person, error) {
	...
	rows, err := p.dbPool.Query(context.Background(),
		"SELECT id, name, age, address, work FROM people")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PersonRepository) Get(id uint64) (model.Person, error) {
	...
	err := p.dbPool.QueryRow(context.Background(),
		"SELECT id, name, age, address, work FROM people WHERE id=$1",
...
func (p *PersonRepository) Update(person *model.Person) error {
	_, err := p.dbPool.Exec(context.Background(),
		"UPDATE people SET name=$1, age=$2, address=$3, work=$4 WHERE id=$5",
